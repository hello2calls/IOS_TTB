...
#import "TPDContactInfoManager.h"
...
@interface TPDContactInfoManagerCopy()<TPContactInfoViewControllerDelegate,ContactHistoryViewControllerDelegate>{
    __weak ContactInfoViewCopyController *_con;
...
static TPDContactInfoManagerCopy *instance = nil;

@implementation TPDContactInfoManagerCopy

+ (void)initialize{
    instance = [[TPDContactInfoManagerCopy alloc] init];
...
- (void)showContactInfoByPersonId:(NSInteger)personId inNav:(UINavigationController *)nav{
    ...
    _phoneNumber = nil;
    _nav = nav;
    ContactInfoViewCopyController *con = [[ContactInfoViewCopyController alloc]init];
...
- (void)showCallHistoryByPersonId:(NSInteger)personId{
    ...
    if (_nav == nil) {
        _nav = [TouchPalDialerAppDelegate naviController];
    }
    if ( _hisCon == nil ){
        [_nav pushViewController:hisCon animated:YES];
        _hisCon = hisCon;
    }else{
        [_nav pushViewController:hisCon animated:YES];
...
- (void)popViewController{
    [_con.navigationController popViewControllerAnimated:YES];
...
- (void)onRightButtonAction{
    ...
    if (_infoType == knownInfo)
        [TPDContactInfoUtil chooseEditDeleteActionByPersonId:_personId presentByNav:_nav];
    else if ( _infoType == unknownInfo )
        [TPDContactInfoUtil chooseAddActionByNumber:_phoneNumber presentByNav:_nav];
}
...
- (void)refreshByPersonDataChanged:(id)personChange{
    ...
    if ( _infoType == knownInfo ){
        ...
        if (isCurrentModified || isAdded)
            [self showContactInfoByPersonId:changedData.person_id inNav:_nav];
    }
