...
@interface TPDMySkinViewController () <UIWebViewDelegate, CootekWebHandlerDelegate>{
...
@implementation TPDMySkinViewController
...
- (void)loadView
{
    ...
    HeadTabBar *titleBar = [[HeadTabBar alloc] initWithFrame:CGRectMake((TPScreenWidth()-184)/2, 7.5+TPHeaderBarHeightDiff(), 184, 30) buttonCount:1];
    titleBar.changeSkinHeadTabBar = YES;
    [titleBar tabBarTitle:@[@"我的个性换肤"]];
    [titleBar setSkinStyleWithHost:self forStyle:@"theme_headtabbar_style"];
    titleBar.delegate = self;
    self.titleBar = titleBar;
    [headBar_ addSubview:titleBar];
    
    [self onClickAtIndexBar:1];
...
- (void)addWebView{
    ...
    if (_commonWebView != nil )
        [self clearWebView];
    NSString *webUrl = [NSString stringWithFormat:@"http://dialer.cdn.cootekservice.com/web/internal/app_skin/index.html?lang=%@&version=%@&skin_version=%@",NSLocalizedString(@"skin_lang",@"cn"),CURRENT_TOUCHPAL_VERSION,LOWEST_SKIN_VERSION_CAN_BE_USED];
