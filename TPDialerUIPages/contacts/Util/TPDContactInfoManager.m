...
@interface TPDContactInfoManager()<ContactInfoViewControllerDelegate,ContactHistoryViewControllerDelegate>{
...
static TPDContactInfoManager *instance = nil;

@implementation TPDContactInfoManager

+ (void)initialize{
    instance = [[TPDContactInfoManager alloc] init];
...
- (void)showContactInfoByPhoneNumber:(NSString *)phoneNumber{
    ...
    if ( _con == nil ){
        [_nav pushViewController:con animated:YES];
        _con = con;
    }else{
        [_nav pushViewController:con animated:YES];
        [FunctionUtility removeFromStackViewController:_con];
