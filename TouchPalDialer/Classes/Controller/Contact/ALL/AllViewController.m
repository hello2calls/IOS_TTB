...
#define PHOTO_BUTTON_TAG (1001)
#define ActivityFamilyUrl @"http://search.cootekservice.com/page_v3/activity_family_detail?_token="
#define ActivityFamilyIndexUrl  @"http://search.cootekservice.com/page_v3/activity_family_index?_token="

@implementation DisplayRequirementsForFilter
@synthesize datas;
@synthesize needAZScrollist;
@synthesize needDisplayNote;
...
- (void)loadView
{
    ...
    // content view
	UITableView *tmp_view_content = [[UITableView alloc] initWithFrame:CGRectMake(0, m_searchbar.frame.size.height, TPScreenWidth(), TPHeightFit(320)) style:UITableViewStylePlain];
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView
{
    ...
    NSLog(@"sections total:%ld", numberOfSections);
...
- (void)onButtonPressed: (SpecialNodeType)type{
    ...
    switch (type) {
        case NODE_MY_FAMILY: {
            [DialerUsageRecord recordpath:PATH_FAMILY_NUMBER kvs:Pair(CONTACT_CLICK_FAMILY, @(1)), nil];
            ...
            [UserDefaultsManager setBoolValue:YES forKey:CONTACT_FAMILY_GUIDE_SHOWN];
            [AllViewController  asyncGetActivityFamilyInfo];
...
+ (void)asyncGetActivityFamilyInfo{
    NSString *urlString = [[NSString stringWithFormat:@"%@%@",ActivityFamilyUrl,[SeattleFeatureExecutor getToken]]stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSURLRequest *request = [NSURLRequest requestWithURL:[NSURL URLWithString:urlString]];
    [NSURLConnection sendAsynchronousRequest:request queue:[[NSOperationQueue alloc] init] completionHandler:^(NSURLResponse * _Nullable response, NSData * _Nullable data, NSError * _Nullable connectionError) {
        if (connectionError ==nil && data!=nil) {
            NSError *error = nil;
            NSDictionary *familyDetailDic = [NSJSONSerialization JSONObjectWithData:data options:(NSJSONReadingMutableLeaves) error:&error];
            if (error ==nil && familyDetailDic != nil) {
                NSDictionary *resultDic = familyDetailDic[@"result"];
                NSNumber *resultErrorCodeNum = resultDic[@"error_code"];
                if (resultErrorCodeNum.integerValue==2000) {
                    NSString *bind_success_list_string = resultDic[@"bind_success_list"];
                    NSError* error = nil;
                    NSData *data = [bind_success_list_string dataUsingEncoding:(NSUTF8StringEncoding)];
                    NSArray *familyDetailArray = [NSJSONSerialization JSONObjectWithData:data options:(NSJSONReadingMutableLeaves) error:&error];
                    
                    [AllViewController saveFamilyInfoWithArray:familyDetailArray];
                }
    ...
    NSString *urlString2 =[[NSString stringWithFormat:@"%@%@",ActivityFamilyIndexUrl,[SeattleFeatureExecutor getToken]]stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSURLRequest *request2 = [NSURLRequest requestWithURL:[NSURL URLWithString:urlString2]];
    [NSURLConnection sendAsynchronousRequest:request2 queue:[[NSOperationQueue alloc] init] completionHandler:^(NSURLResponse * _Nullable response, NSData * _Nullable data, NSError * _Nullable connectionError) {
        if (connectionError ==nil && data!=nil) {
            NSError *error = nil;
            NSDictionary *familyDetailDic = [NSJSONSerialization JSONObjectWithData:data options:(NSJSONReadingMutableLeaves) error:&error];
            if (error ==nil && familyDetailDic != nil) {
                ...
                NSNumber *resultErrorCodeNum = resultDic[@"error_code"];
                if (resultErrorCodeNum.integerValue==2000) {
                    NSNumber *number = resultDic[@"news_num"];
                    cootek_log(@"%@",number);
                    if (number.integerValue != [UserDefaultsManager intValueForKey:ACTIVITY_FAMILY_NEWS_NUMBER defaultValue:0]) {
                        [[NSNotificationCenter defaultCenter] postNotificationName:N_REFRESH_TOUCHPAL_NODE_ALERT object:nil];
                    }
                    [UserDefaultsManager setIntValue:number.integerValue forKey:ACTIVITY_FAMILY_NEWS_NUMBER];
                    [[NSNotificationCenter defaultCenter] postNotificationName:N_REFRESH_TOUCHPAL_NODE_ALERT object:nil];
                }
            };
        }
...
+ (void)saveFamilyInfoWithArray:(NSArray *)arr {
    NSString *bindSuccessListPath = [[AllViewController getFamilyNumberDirPath] stringByAppendingPathComponent:@"bindSuccessListArray.plist"];
    NSString *bindSuccessNumberPath = [[AllViewController getFamilyNumberDirPath] stringByAppendingPathComponent:@"bindSuccessNumberArray.plist"];

    NSMutableArray *numberArray = [NSMutableArray array];
    for (NSDictionary *infodic in arr) {
        NSString *phone = infodic[@"phone"];
        [numberArray addObject:phone];
    }
    [numberArray writeToFile:bindSuccessNumberPath atomically:YES];
    [arr writeToFile:bindSuccessListPath atomically:YES];

}

+ (NSArray *)getNumberArrarFromBindsuccessListArray {
    NSString *bindSuccessNumberPath = [[AllViewController getFamilyNumberDirPath] stringByAppendingPathComponent:@"bindSuccessNumberArray.plist"];
    NSArray *arrar = [NSArray arrayWithContentsOfFile:bindSuccessNumberPath];
    return arrar;
}

+(NSString *)getFamilyNumberDirPath {
    NSString *docmentCommonFileDirPath= [FileUtils getAbsoluteFilePath:@"commonFile"];
    [FileUtils createDir:docmentCommonFileDirPath];
    NSString *CommonFileFamilyNumberDirPath = [docmentCommonFileDirPath stringByAppendingString:@"/familyNumber"];
    [FileUtils createDir:CommonFileFamilyNumberDirPath];
    return CommonFileFamilyNumberDirPath;
}
