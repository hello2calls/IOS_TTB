...
@interface TPGestureSearch(){
    NSMutableArray *searchObjects_;
}
@end
@implementation TPGestureSearch
...
- (id)init
{
    ...
    if (self) {
        ...
        @synchronized (searchObjects_) {
            ...
            search = [[TPGestureNameSearch alloc] init];
            [searchObjects_ addObject:search];
...
-(NSArray *)removeRepeatRecord:(NSArray *)records
{
    ...
    for (SearchItemModel *item in records) {
        if (item.personID > 0) {
            NSString *key = [NSString stringWithFormat:@"%d:%@", item.personID, [item.number digitNumber]];
