 ...
 */
PJ_DECL(pj_status_t) pjmedia_jbuf_create(
					 const pj_str_t *name,
					 unsigned frame_size,
					 unsigned ptime,
					 unsigned max_count,
...
/**
 * Check if the jitter buffer is empty.
 *
 * @param jb        The jitter buffer.
 *
 * @return          PJ_TRUE if it is empty.
 */
PJ_DECL(pj_bool_t) pjmedia_jbuf_is_empty(const pjmedia_jbuf *jb);
